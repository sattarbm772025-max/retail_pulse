...
export function SalesPage() {
  ...
  const cards = [
    ["Total sales", `₹${(summary.data?.total_sales ?? 0).toFixed(2)}`],

    ["Total revenue", `₹${(summary.data?.total_revenue ?? 0).toFixed(2)}`],

    ["Total orders", summary.data?.total_orders ?? 0],

    [
      "Average order value",

      `₹${(summary.data?.average_order_value ?? 0).toFixed(2)}`,
    ],
  ];
  return (
    <DashboardLayout>
    ...
    <Card variant="outlined">
      <CardContent>
        ...
        >
          ...
          >
            <Chip
              label={sale.sales_channel.replace(
                "_",
...
function SaleEditor({
  ...
  const updateLine = (
    ...
    setForm({
      ...
      items: form.items.map((line, current) =>
        current === index
          ? {
              ...line,
    ...
    >
            ...
            <SaleLine
              ...
              remove={() =>
                setForm({
                  ...
                  items: form.items.filter((_, current) => current !== index),
                })
              }
      ...
      <DialogActions>
        ...
        <Button
          disabled={
            !form.customer_name ||
            form.items.some((line) => !line.product_id || line.quantity < 1) ||
            saving
          }
    ...
    <Dialog open onClose={close} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        <Stack spacing={1}>
          ...
          <Typography>
            <b>Date:</b> {new Date(sale.sale_date).toLocaleString()}
          </Typography>

          <Typography>
            <b>Channel:</b> {sale.sales_channel.replace("_", " ")}
          </Typography>

          <Typography>
            <b>Payment:</b> {sale.payment_method.replace("_", " ")}
          </Typography>
