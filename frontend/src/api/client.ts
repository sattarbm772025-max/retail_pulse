...
const ACCESS_KEY = 'retailpulse.access_token'
const REFRESH_KEY = 'retailpulse.refresh_token'
...
api.interceptors.response.use(undefined, async (error) => {
  ...
  if (error.response?.status !== 401 || request?._retry || !tokenStore.getRefresh()) return Promise.reject(error)
  request._retry = true
  refreshing ??= axios.post(`${API_URL}/auth/refresh`, { refresh_token: tokenStore.getRefresh() })
    .then(({ data }) => {
