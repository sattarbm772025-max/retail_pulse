...
def _adjust_stock(
    ...
    if not product:
        raise HTTPException(
            status_code=422,
            detail="Product does not belong to your company"
        )
    ...
    if direction < 0:
        ...
        if product.status != "ACTIVE":
            ...
            raise HTTPException(
                status_code=422,
                detail=f"{product.name} is not available for sales"
            )
