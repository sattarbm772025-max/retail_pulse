...
def generate_forecasts(db, user, period: int):
    products = (
        db.query(Product)
        .filter(Product.company_id == user.company_id, Product.status == "ACTIVE")
        .all()
    )
    if not products:
        raise HTTPException(
            status_code=422, detail="No active products are available for forecasting"
        )
    created = []
    for product in products:
        historical = (
            ...
            .join(Sale)
            .filter(
                Sale.company_id == user.company_id, SaleItem.product_id == product.id
            )
            .scalar()
        )
        if not historical:
        ...
        db.add(
            ForecastHistory(
                ...
                historical_sales=float(historical),
                prediction=prediction,
                accuracy=forecast.confidence_score * 100,
            )
        )
        inventory = (
            db.query(Inventory)
        ...
        if inventory and prediction > inventory.available_stock:
            db.add(
                Notification(
                    ...
                    product_id=product.id,
                    level="FORECAST",
                    message=f"Forecast recommends restocking {product.name}: predicted demand {prediction}, available {inventory.available_stock}.",
                )
            )
        created.append(forecast)
    if not created:
        raise HTTPException(
            status_code=422, detail="Forecasting requires historical sales data"
        )
    create_audit_log(
        db,
        user.company_id,
    ...
    db.commit()
    return {"message": "Forecasts generated", "count": len(created)}


def list_forecasts(db, user, period: int, category_id=None, brand=None, sort="demand"):
    query = (
        db.query(DemandForecast, Product, Inventory)
    ...
    return [
        {
            ...
            "historical_sales": history_value(db, forecast.id),
            "predicted_demand": forecast.predicted_demand,
            "confidence": forecast.confidence_score,
            "recommendation": (
                "IMMEDIATE_RESTOCK"
                if (inventory and forecast.predicted_demand > inventory.available_stock)
...
def history_value(db, forecast_id):
    row = (
        ...
        .filter(ForecastHistory.forecast_id == forecast_id)
