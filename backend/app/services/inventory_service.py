...
def ensure_inventory(db, product: Product) -> Inventory:
...
def create_stock_notifications(
    db, current_user, inventory, previous_status, movement_type, quantity, reason
):
...
def apply_movement(
    ...
    updated_quantity = previous_quantity + quantity_changed
    ...
    inventory.available_stock = updated_quantity - inventory.reserved_stock
    ...
    create_audit_log(
        ...
        current_user.company_id,
        current_user.id,
        actions.get(movement_type, "Inventory Updated"),
        commit=False,
        entity_type="PRODUCT",
        entity_name=product.name,
        quantity_changed=quantity_changed,
...
def get_inventory(
    ...
    if search:
        ...
        query = query.filter(
            or_(Product.name.ilike(keyword), Product.sku.ilike(keyword))
        )
    ...
    if stock_status_filter:
        ...
        query = query.filter(Inventory.stock_status == stock_status_filter.upper())
...
def adjust_inventory(db, current_user, inventory_id, request):
    ...
    if request.adjustment_type == "STOCK_OUT":
        ...
        quantity = (
            request.quantity if request.direction == "INCREASE" else -request.quantity
        )
    else:
        quantity = request.quantity
...
def update_reorder_level(
    ...
    inventory.stock_status = stock_status(inventory.available_stock, reorder_level)
    ...
    create_stock_notifications(
        db, current_user, inventory, previous_status, "REORDER_LEVEL", 0, reason
    )
        ...
        "items": [
            {
                ...
                "performed_by_name": user_names.get(
                    movement.performed_by, "Unknown user"
                ),
...
def summary(db, current_user):
    ...
    return {
        ...
        "total_inventory_quantity": query.with_entities(
...
def charts(db, current_user):

    category_rows = (
        db.query(Category.name, func.sum(Inventory.available_stock))
    ...
    status_rows = (
        db.query(Inventory.stock_status, func.count(Inventory.id))
        .filter(Inventory.company_id == current_user.company_id)
    ...
    movement_rows = (
        db.query(
            ...
            func.sum(InventoryMovement.quantity_changed),
        )
    ...
    return {
        "inventory_by_category": [
            {"category": category, "quantity": int(quantity or 0)}
        ...
        "movement_trend": [
            {"date": str(date), "quantity": int(quantity or 0)}
            for date, quantity in movement_rows
        ],
    }
