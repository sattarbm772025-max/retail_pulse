...
def validate_category(db, category_id: int, company_id: int):
    category = (
        db.query(Category)
    ...
    if not category:
        raise HTTPException(
            status_code=422, detail="Category does not belong to your company"
        )
...
def check_duplicate(db, request, company_id, exclude_id=None):

    sku_query = db.query(Product).filter(
        Product.company_id == company_id, Product.sku == request.sku
    )

    name_query = db.query(Product).filter(
    ...
    if exclude_id:
        ...
        sku_query = sku_query.filter(Product.id != exclude_id)

        name_query = name_query.filter(Product.id != exclude_id)

    if sku_query.first():

        raise HTTPException(
            status_code=409, detail="SKU already exists in this company"
        )
...
def create_product(db, current_user, request):
    ...
    validate_category(db, request.category_id, current_user.company_id)

    check_duplicate(db, request, current_user.company_id)
...
def get_products(
    ...
    ordering = {
        ...
        "price": Product.unit_price.asc(),
        "recent": Product.created_at.desc(),
    }

    products = query.order_by(ordering.get(sort, Product.created_at.desc())).all()
...
def get_product(product_id, db, current_user):
    ...
    product = get_product_or_404(db, product_id, current_user.company_id)
...
def update_product(product_id, request, db, current_user):
    ...
    if old_status != product.status:
        ...
        action = (
            ...
            else f"Product Deactivated: {product.name}"
        )
