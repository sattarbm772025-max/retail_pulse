...
def _summary(db, customer_id: int) -> CustomerPurchaseSummary:
    summary = (
        db.query(CustomerPurchaseSummary).filter_by(customer_id=customer_id).first()
    )
    if summary is None:
...
def _serialize(customer: Customer, summary: CustomerPurchaseSummary) -> dict:
    return {
        ...
        "quantity_purchased": summary.quantity_purchased,
...
def list_customers(
    ...
    sorting = {
        ...
        "since": Customer.created_at.desc(),
        "last_purchase": Customer.updated_at.desc(),
        "spent": Customer.updated_at.desc(),
        "orders": Customer.updated_at.desc(),
    }
    page, page_size = max(page, 1), min(max(page_size, 1), 100)
    total = query.count()
    records = (
        query.order_by(sorting.get(sort, sorting["name"]))
        .offset((page - 1) * page_size)
        .limit(page_size)
        .all()
    )
    return {
        "items": [_serialize(record, _summary(db, record.id)) for record in records],
        "page": page,
        "page_size": page_size,
        "total": total,
...
def customer_summary(db, user):
    ...
    summaries = (
        ...
        .join(Customer, Customer.id == CustomerPurchaseSummary.customer_id)
        .filter(Customer.company_id == user.company_id)
        .all()
    )
    return {
        "total_customers": customers.count(),
        "active_customers": customers.filter(Customer.status == "ACTIVE").count(),
        "inactive_customers": customers.filter(Customer.status == "INACTIVE").count(),
        "vip_customers": customers.filter(Customer.segment == "VIP").count(),
        "total_revenue": sum(row.total_revenue or 0 for row in summaries),
    }
...
def create_customer(db, user, payload):
    ...
    if duplicate:
        raise HTTPException(
            status_code=409,
            detail="Customer email or phone already exists in your company",
        )
    sequence = (
        db.query(Customer).filter(Customer.company_id == user.company_id).count() + 1
    )
    customer = Customer(
        company_id=user.company_id,
        customer_code=f"CUS-{datetime.now().year}-{sequence:05d}",
        **payload.model_dump(),
    )
    db.add(customer)
    db.flush()
...
def change_customer_status(db, user, customer_id: int, status: str, action: str):
    ...
    db.add(
        CustomerTimeline(
            customer_id=customer.id,
            action=action,
            description=f"Customer {status.lower()}",
            performed_by=user.id,
        )
    )
    create_audit_log(
...
def recalculate_customer_summary(db, user, customer_id: int):
    ...
    orders, revenue, first_purchase, last_purchase = (
        db.query(
            func.count(Sale.id),
    ...
    summary.total_revenue = Decimal(str(revenue or 0))
    summary.quantity_purchased = int(quantity or 0)
    summary.average_order_value = (
        Decimal(str(revenue or 0)) / orders if orders else Decimal("0")
    )
...
def customer_detail(db, user, customer_id):
    ...
    sales = (
        db.query(Sale)
        .filter(Sale.company_id == user.company_id, Sale.customer_id == customer.id)
    ...
    data["timeline"] = [
        ...
        for row in db.query(CustomerTimeline)
        .filter(CustomerTimeline.customer_id == customer.id)
...
def record_sale_for_customer(db, user, sale, quantity):
    ...
    summary.total_revenue = Decimal(summary.total_revenue or 0) + sale_total
    summary.quantity_purchased += quantity
    summary.average_order_value = summary.total_revenue / summary.total_orders
    summary.first_purchase_at = summary.first_purchase_at or sale.sale_date
    summary.last_purchase_at = sale.sale_date
    previous_segment = customer.segment
    customer.segment = _segment(summary)
    db.add(
        CustomerTimeline(
            customer_id=customer.id,
            action="FIRST_PURCHASE" if was_new else "PURCHASE",
            description=f"Purchase recorded: {sale.invoice_number}",
            performed_by=user.id,
        )
    )
    if was_new:
        db.add(
            Notification(
                company_id=user.company_id,
                level="CUSTOMER",
                message=f"First purchase from {customer.full_name}",
            )
        )
